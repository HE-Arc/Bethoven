      ...
      <v-col cols="6">
        ...
        <v-btn-toggle borderless v-model="ordering" class="px-2">
          ...
          <v-btn value="asc" @click="finishedTyping">
            ...
            <v-icon right> mdi-arrow-up-bold-circle-outline </v-icon>
          </v-btn>
      ...
      <v-row align="center" justify="center">
        <v-col cols="12">
          <div v-for="user in users" :key="user.id">
            <profil-card :user="user" :clickable="true"> </profil-card>
          </div>
        </v-col>
      </v-row>
    </v-container>
    ...
    async search() {
      ...
      try {
        this.users = await Api.get(
          `users/search/?username=${this.unameQuery}${orderingQuery}`
        );
      } catch (e) {
        console.log(e);
      }
